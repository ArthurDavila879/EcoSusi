...
locaisParaReserva.forEach(local => {
  ...
  marcador.bindPopup(
    `<strong>Tipo:</strong> ${local.tipo}<br>
     ...
     <button id="bt-tipo1" class="btn-confirmar">Confirmar</button>`
)});



const locaisReservados = [
  
 {
  ...
  locaisReservados.forEach(local => {
  const marcador = L.marker([local.lat, local.lng]).addTo(map);
  marcador.bindPopup(
    `<strong>Tipo:</strong> ${local.tipo}<br>
     <strong>Volume:</strong> ${local.volume}<br>
     <strong>Data:</strong> ${local.data}<br>
     <strong>Anexo:</strong> ${local.anexo}<br><br>
     <button id="bt-tipo3" class="btn-excluir">Cancelar</button>`
)});
